...
function createPaging(data) {
	...
	table += "<tr><th>행번호</th><th>이벤트명</th><th>시작일</th><th>종료일</th><th>할인율</th></tr>"
	...
	for(let j= 1; j <= lastPage; j++){
		...
		for(let i=0; i < data.eventList.length; i++){ //=> 범위를 0~9까지 계속 10개씩 가져오는 것이 아니라, data 길이만큼씩 보여주게 해야된다잇!!
			table += "<tr onclick='detail(" + data.eventList[i].eventid + ")'>" +
						"<td style='text-align:center'>" + rownumber +"</td>" +
						"<td style='text-align:center'>" +data.eventList[i].ename +"</td>" +
					    "<td style='text-align:center'>" + data.eventList[i].startdate +"</td>" +
					    "<td style='text-align:center'>" + data.eventList[i].enddate +"</td>" +
					    "<td style='text-align:center'>" + data.eventList[i].salerate +"%</td>" +
				    "</tr>";
					rownumber--;
...
function detail(id){
	...
	let url = "/SellreMarket/adminEventDetailPage.do";
