...
function createPaging(data) {
	...
	table += "<tr><th>행번호</th><th>제목</th><th>작성일</th><th>상태</th></tr>"
	...
	for(let j= 1; j <= lastPage; j++){
		...
		for(let i=0; i < data.questList.length; i++){ //=> 범위를 0~9까지 계속 10개씩 가져오는 것이 아니라, data 길이만큼씩 보여주게 해야된다잇!!
			table += "<tr onclick='detail(" + data.questList[i].inquiryid + ")'>" +
						"<td style='text-align:center'>" + rownumber +"</td>" +
						"<td style='text-align:center'>" +data.questList[i].intitle +"</td>" +
					    "<td style='text-align:center'>" + data.questList[i].insertdate +"</td>" +
					    "<td style='text-align:center'>" + data.questList[i].status +"</td>" +
				    "</tr>";
					rownumber--;
...
function detail(id){
	...
	let url = "/SellreMarket/adminQuestDetailPage.do";
